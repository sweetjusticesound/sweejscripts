...
filters = [
    {
        "filter": "Selected",  # Choose from: "All", "Selected", "SelectedExplicitly", "SelectedImplicitly", 
                               # "WithClipsOnMainPlaylist", "WithAutomationOnMainPlaylist", "Inactive", "InactiveExplicitly", 
                               # "InactiveImplicitly", "Hidden", "HiddenExplicitly", "HiddenImplicitly", "Locked", 
                               # "Muted", "Frozen", "Open", "Online"
        "isInverted": False   # Set to `True` to invert the filter (i.e., it operates as a NOT operator). 
                               # For example, if `filter` is "Selected" and `isInverted` is `True`, 
                               # it will return all tracks that are not selected.
    }
]

# Define whether the filter list is additive (OR) or multiplicative (AND)
is_filter_list_additive = True

# Prepare the arguments
arguments = {
    "filters": filters,
    "isAdditive": is_filter_list_additive
}
...
# Prepare the message
message = f'sweejhelper://proToolsFunction/getTrackListWithFilters/{request_id}?arguments={arguments_encoded}'

# Send a request to get the track list with filters
